// https://mp.weixin.qq.com/s/3zacqN5bjNIccLNuWJN-UA
...
func Rows[T any](ctx context.Context, db *sqlx.DB, query string, args ...any) ([]T, error) {
...
// So had better to tag all fields with db tag to avoid unexpected behavior.
func Row[T any](ctx context.Context, db *sqlx.DB, query string, args ...any) (T, error) {
	var result T
...
func RowsMap(ctx context.Context, db *sqlx.DB, query string, args ...any) ([]map[string]any, error) {
	rows, err := db.QueryContext(ctx, query, args...)
	defer rows.Close()
	if err != nil {
		return nil, err
...
func RowMap(ctx context.Context, db *sqlx.DB, query string, args ...any) (map[string]any, error) {
	rows, err := db.QueryContext(ctx, query, args...)
	defer rows.Close()
	if err != nil {
		return nil, err
